...
const checkSchemeId = async (req, res, next) => {
 ...
 try{
  const validateSchemeId = await db('schemes').where({scheme_id}).first()
  if (!validateSchemeId){
...
const validateStep = (req, res, next) => {
  ...
  if (!instructions || !instructions.trim() || typeof instructions !== 'string' || typeof step_number !== 'number' || isNaN(step_number) || step_number < 1){
